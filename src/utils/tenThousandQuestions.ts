...
export function getTenThousandQuestion(index: number, studentClass: string = 'Lớp 6'): Question {
  ...
  const uniqueRes = ensureUniqueOptions(options, correctAnswer);
...
export function ensureUniqueOptions(options: string[], correctAnswer: string): { options: string[], correctAnswer: string } {
  const correctIdx = correctAnswer.charCodeAt(0) - 65; // 0 for A, 1 for B...
  ...
  for (let i = 0; i < options.length; i++) {
    ...
    let val = newOptions[i];
    let offset = 1;

    // Guard safety if option value is undefined or blank
    if (!val) {
      val = "Lựa chọn bổ sung";
    }
    ...
    while (seen.has(val)) {
      if (val.includes('=')) {
        ...
        if (numPart.includes('/')) {
          const fracParts = numPart.split('/');
          const num = parseInt(fracParts[0]) || 1;
          const den = parseInt(fracParts[1]) || 2;
          val = `${parts.slice(0, -1).join('=')}= ${num + offset}/${den}`;
        } else {
          const num = parseInt(numPart);
          if (!isNaN(num)) {
            val = `${parts.slice(0, -1).join('=')}= ${num + offset}`;
          } else {
            val = `${val} (${offset})`;
          }
        }
  ...
  return { options: newOptions, correctAnswer };
}
